...
$(document).ready(function () {
    ...
    $.ajax({url: '../php/getRequirementsExcel.php?',
        type: 'POST',
        data: {"requirement": requirement},
        dataType: 'json',
        success: function (response) {
            alert(response);
            //var idtarea=response[index]["IdTarea"];
            $.each(response, function (index) {
                if (response[index]["IdTarea"]) {
                $("#cd-timeline").append('<div class="cd-timeline-block">' +
                        ...
                        '<a href="#0" data-line="' + response[index]["IdTarea"] + '" onclick="newUser(this)" class="cd-read-more">Tarea</a>' +    
